...
def fix_gt_entries_camera_names(mongo_uri: str, db_name: str, collection_name: str, start_date: str, limit: int):
    """
    Corrects the 'camera' field in the gt_tools.entries collection by extracting
    the full camera name from the 'entry_id' or '_id' field.
    """
    ...
    for i, doc in enumerate(docs_to_process):
        ...
        # To get the camera name, split the entry_id by '-' and remove the last two parts.
...
if __name__ == "__main__":
    # Set default connection string to the one used for the manifest-editor
    default_mongo_uri = os.getenv("REID_MONGODB_URI", "mongodb://teknoir:change-me@localhost:37017")

    parser = argparse.ArgumentParser(
        description="Correct the 'camera' field in the gt_tools.entries collection by extracting it from the entry_id."
    )
    parser.add_argument(
        "--mongo-uri",
    ...
    parser.add_argument("--collection", default="entries", help="Collection name.")
